...
The function should display a message in the following format: 'Error! {error_msg}.'
...
os.chdir(os.path.expanduser("~\Desktop\Com411\data"))
print(os.getcwd())
with open('covid_19_data.csv', 'r') as COVID19:
...
#Task 2 - display an error message
def error(msg = "oops, please check your input or contact an admin"):
    print("Error! {error}")
pass

#Task 3
def value(range=(-1,101,1)):
    pass



def progress(operation, status):
    operation = getpass
    if value == "0":
        status = "has started"
    elif value == range(0,100,1):
...
#from __main__ import values[5]



def total_records(num_records):
    len(list(COVID19))
    print('There are {num_records} records in the data set')
